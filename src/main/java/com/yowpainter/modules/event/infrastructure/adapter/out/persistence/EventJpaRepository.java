...
public interface EventJpaRepository extends JpaRepository<Event, UUID> {
    ...
    @Query("SELECT e FROM Event e WHERE e.status = com.yowpainter.modules.event.domain.model.EventStatus.PUBLISHED AND e.startDateTime > :now ORDER BY e.startDateTime ASC")
    List<Event> findUpcomingEvents(@Param("now") LocalDateTime now);

    @Query("SELECT e FROM Event e WHERE e.status = com.yowpainter.modules.event.domain.model.EventStatus.PUBLISHED AND (LOWER(e.name) LIKE LOWER(CONCAT('%', :query, '%')) OR LOWER(e.description) LIKE LOWER(CONCAT('%', :query, '%')) OR LOWER(e.location) LIKE LOWER(CONCAT('%', :query, '%')))")
